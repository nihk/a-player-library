...
import library.ui.databinding.TrackItemBinding
import library.ui.databinding.TracksFragmentBinding
...
class TracksPickerFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(
        ...
        val adapter = TracksAdapter(listOf(auto) + trackOptions) { trackOption ->
            when (trackOption) {
                is TrackOption.Auto -> setFragmentResult(KEY_PICK_RESULT, bundleOf(KEY_RENDERER_INDEX to trackOption.rendererIndex))
                is TrackOption.SingleTrack -> setFragmentResult(KEY_PICK_RESULT, bundleOf(KEY_TRACK_INFO to trackOption.trackInfo))
            }
